...
object Tasks extends App {
  ...
  def codesSumOf3(s: String): Long =
    s.codePoints().asLongStream().toArray.product
  ...
  def pow_ch2_tsk10(x: Int, i: Int): Int =
    if (i == 0) 1
      ...
      pow_ch2_tsk10(x, i / 2) * pow_ch2_tsk10(x, i / 2)
  ...
  def arr_ch3_tsk6_1(ar: Array[Int]): Array[Int] =
    scala.util.Sorting.stableSort(ar, _ > _)
  ...
  def map_ch4_tsk9_lteqgt(ar: Array[Int], v: Int): (Int, Int, Int) =
    (ar.count(_ < v), ar.count(_ == v), ar.count(_ > v))
  println(
    map_ch4_tsk9_lteqgt(Array(2, 3, 5, 6, 9, 5, 6, 5, 4, 6, 2, 65899, 0), 8)
  )
