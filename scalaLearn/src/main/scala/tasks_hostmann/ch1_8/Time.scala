...
class Time(private var hours:Int, private var minutes:Int) {
  ...
  def before(thisT:Time, otherT: Time):Boolean = (thisT.hours*60+thisT.minutes)<(otherT.hours*60+otherT.minutes)
  hours= innerField/60
  minutes = innerField%60
  def hour = hours
  override def toString: String = "hours:"+this.hours+", minutes: "+this.minutes
}
