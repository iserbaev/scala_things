...
object TwitterFilterToCats extends App {
  ...
  val rT: Filter[String, Unit]           = (r, s) => fT1(r, identityT).flatMap(s2 => fT2(s2, identityT)).flatMap(s)
  ...
  val printC: CatsService[String, Unit]      = Kleisli(s => Future.successful(println(s)))
  val sUpC: CatsService[String, String]      = Kleisli(s => Future.successful(s.toUpperCase))
  val sExC: CatsService[String, String]      = Kleisli(s => Future.successful(s + "!"))
  ...
  val rC: CatsFilter[String, Unit] = (r, k) => fC1(r, identityC).flatMap(fC2(_, identityC)).flatMap(s => k.apply(s))
