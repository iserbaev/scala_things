...
/**
  *  1) Cжать последовательность интов
  ...
  *  2) восстановаить исходную последовательность из сжатой
  */
object ZipSeq extends App {
  ...
  val ((last, count), acc) =
    in.tail.foldLeft(((in.head, 1), Seq.empty[(Int, Int)])) {
      case (((prev, count), acc), el) =>
        if (el == prev) {
          ((prev, count + 1), acc)
        } else {
          ((el, 1), acc.:+((prev, count)))
